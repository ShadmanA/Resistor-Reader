/*  
This program prompts for the colors of three bands of a resistor, and displays the * value of the  resistance in kilo-ohms based on a color code 
*/  
#include <stdio.h> 
...
do{ 
	...
	printf("\n");  
  } while(reply != 'n' && reply == 'y'); /* (1 mark) */
return(0);
}
/* function takes as input a list of strings, its size, and a target string. Then, searches the list for the  target and returns as its value the subscript of the target in the list. It returns -1 if target is not found. */  
int search(const char COLOR_CODES[][SUB_2], const char target[], int size){ 
	...
	for(i=0; (i<size) && (found!=1); i++) { /* (1 mark) */ 
			for(j = 0; j < length; j++)  
